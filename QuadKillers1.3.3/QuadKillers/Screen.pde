...
void spawnButtons() {
  if (screen ==1) { //title screen
    ...
    buttons.add(new DifficultyMenu());
  } else if (screen == 4) {
    ...
    buttons.add(new WorldSelect("World 2", "WhiteWorld", 300));
    buttons.add(new WorldSelect("Testing", "TestWorld", 400));
    buttons.add(new WorldSelect("Infinite Magic", "PurpleWorld", 500));
    buttons.add(new GoBack());
  } else if (screen == 8) {
    buttons.add(new DifficultySelect("Normal", "normal", 200));
    buttons.add(new DifficultySelect("Hitless", "nohit", 300));
    buttons.add(new DifficultySelect("Double Damage", "doublehit", 400));
    buttons.add(new GoBack());
  }
}
void screenLoop() {
...
class DifficultySelect extends Button {
  String difficulty;
  DifficultySelect(String displayText, String difficulty, float h) {
    startTime = millis;
    this.text = displayText;
    pos.set(0.5*width, h);
    size.set(200, 50);
...
class scrollText {
  ...
  void scroll (float scroll) {
    this.scroll = constrain(this.scroll+scroll, 0,1800);
